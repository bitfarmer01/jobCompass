...
- **EXTRA IMPORTANT**: This project uses Tailwind CSS v4. Design tokens are defined with the `@theme` directive in `app/globals.css` — there is no `tailwind.config.ts`. See `context/ui-tokens.md` and `context/ui-rules.md`.