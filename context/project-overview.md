...
JobCompass is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using AI. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
JobCompass eliminates all of that preparation work. The agent finds the jobs, scores them intelligently against the user's actual skills, and researches each company so the user arrives at every application fully informed. The user just decides which ones to apply to and clicks.
...
- Two options on upload:
  - "Extract from Resume" → AI parses resume and auto-fills profile form fields
  - "Skip" → resume stored as-is, profile unchanged
- User can manually edit any profile field at any time
- User can generate a clean professional PDF resume from their current profile data using AI
...
- Match score section:
  ...
  - Match reason paragraph from AI
...
- AI synthesizes all extracted content into structured dossier
- Dossier displayed on job details page
- If company site cannot be found — AI generates best dossier from company name and job description alone
...
- Resume PDF upload with optional profile auto-fill via AI
- Resume PDF generation from profile data using AI
...
- Company Research Agent — single Browserbase session browses company public pages, AI builds dossier
...
- AI match scores feel accurate and the reasoning makes sense
