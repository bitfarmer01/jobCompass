...
- Resume PDF uploaded to InsForge Storage at `resumes/{user_id}/resume.pdf` — **DONE in 06. Bucket access model RESOLVED = PRIVATE.** No `upsert` option in SDK v1.3.1, so `remove()` then `upload()` (File/Blob only). Served via the authenticated `GET /api/resume` route (no public URL; no `createSignedUrl` in the SDK).
- resume_pdf_url saved to profiles table after upload — stores the storage **path** (`{user_id}/resume.pdf`), not a URL, since the bucket is private.
- is_complete set to true when all required fields are filled (shared helper `lib/profile-completion.ts`)
- ~~Completion percentage and missing fields calculated and saved~~ — **calculated (derived), NOT saved.** The `profiles` table has only `is_complete` (no `%`/`missing_fields` columns; schema frozen in 04). % and missing labels are derived on read/client.
...
Extract from Resume button — NIM reads uploaded PDF and auto-fills profile form fields.
...
- NIM reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using NIM.

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- NIM generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders NIM output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - NIM scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from NIM
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to NIM synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **NIM synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, NIM synthesizes from job description and profile alone.
